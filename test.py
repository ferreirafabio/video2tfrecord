...
class Testvideo2tfrecord(unittest.TestCase):
  ...
  def test_example(self):
    ...
    convert_videos_to_tfrecord(source_path=in_path, destination_path=out_path, n_videos_in_record=n_videos_per_record, n_frames_per_video=n_frames,
                               n_channels=num_depth, dense_optical_flow=False, file_suffix="*.mp4")

    filenames = gfile.Glob(os.path.join(out_path, "*.tfrecords"))
    n_files = len(filenames)

    self.assertTrue(filenames)
    self.assertEqual(n_files * n_videos_per_record, get_number_of_records(filenames))
...
def read_and_decode(filename_queue):
  ...
  for imageCount in range(n_frames):
  ...
  with tf.Session() as sess_valid:
    ...
    try:
      while True:
        ...
        assert np.shape(video) == (1, n_frames, height, width, num_depth), "shape in the data discriminates from the expected shape"
        num_examples += 1
